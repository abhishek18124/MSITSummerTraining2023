...
TreeNode* buildBBST(int* A, int s, int e) {
	...
	if (s > e) {
		// build an empty BST using A[...] = {}
		return NULL;
	}

	// recursive case

	// build a balanced BST using A[s...e]

	int m = s + (e - s) / 2;

	TreeNode* root = new TreeNode(A[m]);

	// recursively, build the leftSubtree using A[s...m-1]

	root->left = buildBBST(A, s, m - 1);


	// recursively, build the rightSubtree using A[m-1...e]
